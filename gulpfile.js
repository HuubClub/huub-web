...
gulp.task('sass', function() {
  ...
  .pipe(cleanCSS())
  .pipe(gulp.dest('dist/source-files/unio-css-min-files'))
});
...
gulp.task('buildexpandedcss', function() {
  ...
  .pipe(autoprefixer(['last 15 versions', 'ie >= 7', 'Firefox < 20']))
  .pipe(gulp.dest('dist/source-files/unio-css-files'));
});
...
gulp.task('libs', function() {
  return gulp.src([
    ...
    'bower_components/swiper/package/js/swiper.js',
...
gulp.task('buildexpandedlibs', function() {
  ...
  .pipe(concat('libs.js'))
  .pipe(gulp.dest('dist/source-files/unio-js-files'));
});
...
gulp.task('build', ['clean', 'sass', 'buildhtml', 'pics', 'libs', 'buildexpandedcss', 'buildexpandedlibs'], function() {
  ...
  var buildUnminifiedJs = gulp.src([
    ...
    'app/js/maps/google-map.js',
    'app/js/gallery-init.js',
    'app/js/unio-custom.js'
  ])
  .pipe(gulp.dest('dist/source-files/unio-js-files'));
